...
* V1: Basic setup commands added for HUMPRO 900 transceiver
*
* NOTES:
* - Need to change how it receives the data to be compatible with UART RX interrupt 
*/
...
uint8_t * setupTransceiver(){
	...
	TX0Write(TXPower[0],4);
	TX0Write(UARTBaud[0],4);
	TX0Write(addMode[0],4);
	TX0Write(showVer[0],4);
	TX0Write(CMDHold[0],4);
	TX0Write(compat[0],4);
